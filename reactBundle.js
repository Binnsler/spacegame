...
var Home = function (_React$Component) {
    ...
    _createClass(Home, [{
        ...
        value: function startGame() {
            ...
            var game = new _game2.default(this.refs.canvas, context);
        ...
        value: function render() {
            return _react2.default.createElement(
                ...
                _react2.default.createElement("canvas", { id: "game", ref: "canvas", width: "320", height: "480" })
            );
        }
    }]);
...
var _engine = __webpack_require__(79);

var _engine2 = _interopRequireDefault(_engine);

var _PlayerShip = __webpack_require__(80);

var _PlayerShip2 = _interopRequireDefault(_PlayerShip);

var _spriteData = __webpack_require__(82);
...
var Game = function Game(canvas, context) {
    ...
    this.initialize = function () {
        _this.spriteSheet = new _engine2.default.SpriteSheet();
        _this.canvas = canvas;
    ...
    this.startGame = function () {
        _this.spriteSheet.load(_spriteData2.default, function () {});

        _this.setBoard(0, new _engine2.default.StarField(_this, 4, 0.4, 100, true));
        _this.setBoard(1, new _engine2.default.StarField(_this, 8, 0.6, 100));
        _this.setBoard(2, new _engine2.default.StarField(_this, 16, 1.0, 25));
        _this.setBoard(3, new _engine2.default.TitleScreen(_this, "The Final Frontier", "Press spacebar to start playing!", _this.playGame));
    };

    //////
    // Handle Input
    //////
    this.setupInput = function (game) {
    ...
    this.loop = function (game) {
        ...
        for (var i = 0; i < _this.boards.length; i++) {
            if (_this.boards[i]) {
                _this.boards[i].step(dt);
                _this.boards[i] && _this.boards[i].draw(game.context);
            }
        };

        setTimeout(function () {
            return _this.loop(game);
        }, 30);
    };

    this.setBoard = function (num, board) {
        _this.boards[num] = board;
    };

    this.playGame = function () {
        var board = new _engine2.default.GameBoard();

        board.add(new _PlayerShip2.default(_this, _this.spriteSheet));

        _this.setBoard(3, board);
    };
};
...
var Engine = {
    "SpriteSheet": function SpriteSheet() {
        ...
        this.load = function (spritesData, callback) {
            _this.sprites = spritesData;
            _this.image = new Image();
            _this.image.onload = callback;
            _this.image.src = "/src/content/images/sprites.png";
        };

        this.draw = function (ctx, spriteName, drawX, drawY, frame) {
            var sprite = _this.sprites[spriteName];

            if (!frame) {
                frame = 0;
            }

            ctx.drawImage(_this.image, sprite.sx + frame * sprite.w, sprite.sy, sprite.w, sprite.h, drawX, drawY, sprite.w, sprite.h);
        };
    },
    ...
    "StarField": function StarField(Game, speed, opacity, numStars, clear) {
        ...
        for (var i = 0; i < numStars; i++) {
            starsCxt.fillRect(Math.floor(Math.random() * stars.width), Math.floor(Math.random() * stars.height), 2, 2);
        }
        ...
        this.draw = function (context) {
            ...
            if (intOffset > 0) {
                context.drawImage(stars, 0, remaining, stars.width, intOffset, 0, 0, stars.width, intOffset);
            }

            // Draw the bottom half of the starfield
            if (remaining > 0) {
                context.drawImage(stars, 0, 0, stars.width, remaining, 0, intOffset, stars.width, remaining);
            }
        };
    ...
    "GameBoard": function GameBoard() {
        ...
        this.remove = function (obj) {
            return _this2.removed.push(obj);
        };

        this.resetRemoved = function () {
            return _this2.removed = [];
        };

        this.finalizeRemoved = function () {
            for (var i = 0; i < _this2.removed.length; i++) {
                var index = _this2.objects.indexOf(_this2.removed[i]);

                if (index != -1) {
                    _this2.objects.splice(index, 1);
                }
            }
        };
        ...
        this.detect = function (func) {
            for (var i = 0; i < _this2.objects.length; i++) {
                if (func.call(_this2.objects[i])) {
                    return _this2.objects[i];
                }
            }
        ...
        this.step = function (dt) {
            _this2.resetRemoved();
            _this2.iterate("step", dt);
            _this2.finalizeRemoved();
        };

        this.draw = function (cxt) {
            return _this2.iterate("draw", cxt);
        };

        // Check edges to see if possible to overlap. Notice !bang operator to negate
        this.overlap = function (obj1, obj2) {
            return !(obj1.y + obj1.h - 1 < obj2.y || obj1.y > obj2.y + obj2.h - 1 || obj1.x + obj1.w - 1 < obj2.x || obj1.x > obj2.x + obj2.w - 1);
        };

        // Check for collision, returning obj ( true ) or false
        this.collide = function (obj, type) {
            return _this2.detect(function () {
                if (obj != _this2) {
                    var col = (!type || _this2.type & type) && _this2.board.overlap(obj, _this2);

                    return col ? _this2 : false;
                }
            });
        };
    }
...
var _PlayerMissle = __webpack_require__(81);
...
var PlayerShip = function PlayerShip(Game, SpriteSheet) {
    ...
    this.w = SpriteSheet.sprites["ship"].w;
    this.h = SpriteSheet.sprites["ship"].h;
    ...
    this.step = function (dt) {
        ...
        if (_this.x < 0) {
            _this.x = 0;
        } else if (_this.x > Game.width - _this.w) {
            _this.x = Game.width - _this.w;
        }

        // Missle launch check
        _this.reload -= dt;

        if (Game.keys["fire"] && _this.reload < 0) {
            Game.keys["fire"] = false;

            _this.reload = _this.reloadTime;

            _this.board.add(new _PlayerMissle2.default(Game, _this.spriteSheet, _this.x, _this.y + _this.h / 2));
            _this.board.add(new _PlayerMissle2.default(Game, _this.spriteSheet, _this.x + _this.w, _this.y + _this.h / 2));
        }
    };

    this.draw = function () {
        SpriteSheet.draw(_this.cxt, "ship", _this.x, _this.y);
    };
}; // Components
...
var PlayerMissle = function PlayerMissle(Game, SpriteSheet, x, y) {
    ...
    this.w = this.spriteSheet.sprites["missle"].w;
    this.h = this.spriteSheet.sprites["missle"].h;
...
PlayerMissle.prototype.draw = function draw(cxt) {
    this.spriteSheet.draw(cxt, "missle", this.x, this.y);
};
...
module.exports = {"ship":{"sx":1,"sy":0,"w":36,"h":42},"missle":{"sx":0,"sy":30,"w":2,"h":10},"purpleEnemy":{"sx":37,"sy":0,"w":42,"h":42},"redEnemy":{"sx":79,"sy":0,"w":37,"h":42},"greyEnemy":{"sx":117,"sy":0,"w":40,"h":42},"greenEnemy":{"sx":159,"sy":0,"w":34,"h":42}}
